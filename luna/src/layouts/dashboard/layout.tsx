...
export function MainLayout({
  ...
  const renderMain = () => (
    <MainSection 
      ...
      sx={{
        ...slotProps?.main?.sx,
        pb: { xs: isMobile ? 8 : 0, [layoutQuery]: 0 }, // Add bottom padding for mobile nav
      }}
    >
      {children}
    </MainSection>
  ...
  const renderMobileBottomNav = () => {
    ...
    const bottomNavItems = [
      { label: '대시보드', value: '/dashboard', icon: 'solar:widget-bold' },
      { label: '채용공고', value: '/jobs', icon: 'solar:case-round-bold' },
      { label: '이력서', value: '/resume', icon: 'solar:document-text-bold' },
      { label: '내정보', value: '/profile', icon: 'solar:user-bold' },
    ];
